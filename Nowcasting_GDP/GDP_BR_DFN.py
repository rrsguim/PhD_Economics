...
def main(unused_argv):
  ...
  excel = xlrd.open_workbook('GDP_BR_54_1996_2017.xls') #upload xls
  ...
  #x = dataset[:,1:dataset.shape[1]] # BR_54
  x = dataset[:,25:dataset.shape[1]] # BR_30
  ...
  plt.savefig('DFN_GDP_BR_learning_curves.png')
  plt.close()
  ...
  plt.xlabel('Time')
  plt.ylabel('19?? = 100')
  plt.plot(time_axis, y.T, '-', label='BR GDP') #target
  plt.plot(time_axis, y_predictions.T, '-', label='DFN') #model
  plt.legend()
  plt.savefig('DFN_GDP_BR_outputmodel_X_target.png')
  plt.close()
  ...
  plt.legend()
  plt.savefig('DFN_GDP_BR_error.png')
  plt.close()

  #output model to csv file
  a = np.asarray(y_predictions)
  np.savetxt("DFN_GDP_BR_output.csv", a, delimiter=",")
