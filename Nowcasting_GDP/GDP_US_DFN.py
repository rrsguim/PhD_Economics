...
def main(unused_argv):
  ...
  excel = xlrd.open_workbook('GDP_US_38_dnn_1960_2017.xls') #upload xls data - target + 38 features
  #excel = xlrd.open_workbook('GDP_US_111_dnn_1960_2017.xls') #upload xls data - target + 111 features
  ...
  plt.savefig('DFN_GDP_US_learning_curves.png')
  plt.close()
  ...
  plt.xlabel('Time')
  plt.ylabel('19?? = 100')
  plt.plot(time_axis, y.T, '-', label='US GDP') #target
  plt.plot(time_axis, y_predictions.T, '-', label='DFN') #model
  plt.legend()
  plt.savefig('DFN_GDP_US_outputmodel_X_target.png')
  plt.close()
  ...
  plt.legend()
  plt.savefig('DFN_GDP_US_error.png')
  plt.close()

  #output model to csv file
  a = np.asarray(y_predictions)
  np.savetxt("DFN_GDP_US_output.csv", a, delimiter=",")
